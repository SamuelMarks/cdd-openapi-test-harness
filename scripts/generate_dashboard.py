#!/usr/bin/env python3
"""Generates the ECOSYSTEM_COMPLIANCE_OAS_3_2_0.md dashboard.

This script parses the COMPLIANCE.md files in each CDD toolchain submodule
to evaluate OpenAPI 3.2.0 feature support, and generates a consolidated
markdown dashboard with a support matrix.
"""
...
def analyze_compliance(file_path):
    """Analyzes a COMPLIANCE.md file to determine feature support status.

    Args:
        file_path (str): The path to the COMPLIANCE.md file to analyze.

    Returns:
        dict: A dictionary mapping feature names to their support status ("✅" or "❌").
    """
    ...
    if (
        ...
        in text_lower
        or "100% compliance achieved" in text_lower
        or ("fully compliant" in text_lower and "full support" in text_lower)
    ):
        status = {f: "✅" for f in FEATURES}
    ...
    for line in content.split("\n"):
        ...
        if (
            re.match(r"^#*\s*(Currently )?[Ss]upported.*", line, re.IGNORECASE)
            or re.match(r"^#*\s*Status.*", line, re.IGNORECASE)
            or re.match(r"^#*\s*Support Coverage", line, re.IGNORECASE)
            or re.match(r"^Currently supported features:", line)
        ):
            in_supported = True
            in_pending = False
        elif re.match(
            r"^#*\s*(To Be Implemented|Features pending|Work in Progress|Next Steps|Pending).*",
            line,
            re.IGNORECASE,
        ) or re.match(r"^Features pending:", line):
            in_supported = False
            in_pending = True
            ...
            for kw in kws:
                if kw in line_lower:
                    if "✅" in line:
                        ...
                        "fully supported" in line_lower
                        or "fully implemented" in line_lower
                    ):
                        status[f] = "✅"
                    elif (
                        "supported" in line_lower and not "not supported" in line_lower
                    ):
    ...
    if (
        "cdd-java" in file_path
        and "parsing core models, routes, responses" in text_lower
    ):
